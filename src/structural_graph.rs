...
pub enum ParseError<'a> {
    ...
    RedefinitionError (CircuitNode),
    UndefinedNameError (Rc<str>),
}
...
impl StructuralGraph {
    pub fn parse(input: &str) -> Result<StructuralGraph, ParseError>
    {
        ...
        for Entry{entry_type, name, adjacency_list} in nodes.into_iter() {
           let c = match entry_type {
               EntryType::DataReg => {
                   ...
                   lut.insert(s0, s0i);
                   ret.inner.add_edge(cni, s0i, Channel{initial_phase: ChannelPhase::ReqNull, is_internal: true});
                   adjacency.push((s0i, Channel{initial_phase: ChannelPhase::ReqData, is_internal: true}, vec![s1.clone()]));
           ...
           if let Some(_) = lut.insert(c.name(), ni) {
               return Err(ParseError::RedefinitionError(c))
           }
           adjacency.push((ni, Channel{initial_phase: ChannelPhase::AckNull, is_internal: false}, adjacency_list));
        }
        ...
        for (ni, channel, adjacency_list) in adjacency.into_iter() {
            for x in adjacency_list.into_iter() {
                if let Some(xi) = lut.get(x.as_ref()) {
                    ...
                    return Err(ParseError::UndefinedNameError(x.clone()))
                }
            }
        }
