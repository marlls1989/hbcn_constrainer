...
mod tests {
    ...
    fn test_multithreaded_gag_sharing() {
        ...
        // Collect all handles
        let gag_handles: Vec<_> = handles.into_iter()
            .map(|h| h.join().expect("Thread should complete"))
            .collect();

        // Check results - at least some should succeed
        let successful_handles: Vec<_> = gag_handles.into_iter()
            .filter_map(|h| h.ok())
            .collect();
        
        // Most threads should succeed since there's an existing gag to share
        assert!(!successful_handles.is_empty(), "At least some threads should get gag handles when one exists");
        ...
        println!("Successfully created {} thread-safe gag handles sharing the same instance", 
                 successful_handles.len() + 1); // +1 for initial_handle
    }
